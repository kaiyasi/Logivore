...
Sprachen: Deutsch | [English](../api.md) | [繁體中文](../zh-tw/api.md) | [简体中文](../zh-cn/api.md)

Dieser Leitfaden liefert umfassende Informationen für Entwickler:innen, die Logivore erweitern,
integrieren oder zu dessen Codebasis beitragen möchten.

## Inhaltsverzeichnis

- [Architekturübersicht](#architekturübersicht)
- [Kernkomponenten](#kernkomponenten)
- [Cog-Entwicklung](#cog-entwicklung)
- [Internationalisierung (i18n)](#internationalisierung-i18n)
- [Konfigurationssystem](#konfigurationssystem)
- [Embed-Formatierung](#embed-formatierung)
- [Systemüberwachungs-API](#systemüberwachungs-api)
- [Docker-Integrations-API](#docker-integrations-api)
- [SSL-Verwaltungs-API](#ssl-verwaltungs-api)
- [Alarm-System-API](#alarm-system-api)
- [Fehlerbehandlung](#fehlerbehandlung)
- [Ereignissystem](#ereignissystem)
- [Test-Framework](#test-framework)

## Architekturübersicht

Logivore verwendet eine modulare Architektur auf Basis des Cog-Systems von Discord.py.
So werden Verantwortlichkeiten sauber getrennt und Erweiterungen bleiben übersichtlich.

### Projektstruktur
```
Logivore/
├── main.py                 # Bot-Initialisierung und Kern-Setup
├── config/
│   ├── bot_config.json    # Hauptkonfigurationsdatei
│   └── .env               # Umgebungsvariablen
├── cogs/                  # Modulare Bot-Funktionen
...
1. Modulare Cogs: Jede Funktion als eigenständiges Modul
2. Async-first: Nicht-blockierende Ausführung mit async/await
3. Internationalisierung: Vollständige Mehrsprachigkeit von Beginn an
4. Konfigurationsgetrieben: Verhalten über JSON konfigurierbar
5. Fehlerrobust: Umfassende Fehlerbehandlung und flexible Degradierung
...
class ConfigManager:
    ...
    def set(self, key: str, value: Any) -> None:
        """Werte mit Punktnotation schreiben"""
        keys = key.split('.')
        config = self._config
...
class ExampleCog(commands.Cog):
    ...
    async def example_command(self, interaction: discord.Interaction):
        """Beispiel für einen Slash-Command"""
        try:
...
## Systemüberwachungs-API

### SystemMonitor: Gesamterfassung
...
### CPU, Speicher, Datenträger, Netzwerk (Auszüge)
...
### CPU‑Prozentsatz aus Docker‑Stats
...
### Nginx Proxy Manager (NPM) Auth (Kurz)
...
Logik: periodische Checks, Vergleich mit Schwellwerten, Versand in konfigurierte Kanäle.

## Fehlerbehandlung

- Einheitliche Embed‑Antworten (Titel, Farbe, Footer) via `EmbedFormatter`
- Nutzerfreundliche Meldungen, technische Details geloggt
- Fallbacks und sichere Degradierung bei Teilfehlern
...
- Ereignisse bei Statusänderungen (z. B. Container neu gestartet)
- Optionale Listener zur Weiterleitung (Embed, Logs, Webhooks)

## Test-Framework

- Pytest mit `pytest-asyncio` für asynchrone Pfade
- Mocks für Docker/Netzwerk/Dateisystem
- Fokus: erst modulnahe Tests, dann Integration
